...
data <- data %>%
  mutate(
    Group = case_when(
      AnimalNum %in% susAnimalNums ~ "SUS",
      AnimalNum %in% conAnimalNums ~ "CON",
      TRUE ~ "RES"
    ),
    Sex = case_when(
      AnimalNum %in% sexAnimals$males ~ "m",
      AnimalNum %in% sexAnimals$females ~ "f",
      TRUE ~ ""
    )
  ) %>%
  filter(!AnimalNum %in% exclAnimalNums)  # Exclude animals
...
data <- data %>%
  mutate(
    DateTime = as.POSIXct(DateTime, format = "%d.%m.%Y %H:%M"),  # Convert to datetime
    Phase = case_when(
      format(DateTime, "%H:%M") >= "18:30" | format(DateTime, "%H:%M") < "06:30" ~ "Active",
      TRUE ~ "Inactive"
    ),
    PriorActive = if_else(format(DateTime, "%H:%M") >= "16:30" & format(DateTime, "%H:%M") < "18:30", "TRUE", "FALSE"),
